...
cat ${KAFKA_HOME}/config/log4j.properties.template | sed \
  -e "s|{{KAFKA_HOME}}|${KAFKA_HOME:-/opt/kafka}|g" \
...
cat ${KAFKA_HOME}/config/server.properties.template | sed \
  ...
  -e "s|{{KAFKA_ADVERTISED_HOST_NAME}}|${KAFKA_ADVERTISED_HOST_NAME:-$KAFKA_HOSTNAME}|g" \
  -e "s|{{KAFKA_CLIENT_PORT}}|${KAFKA_CLIENT_PORT:-9092}|g" \
